...
>
  ...
  <p
    v-if="additionalFamilyList.length === 0"
    class="mt-8 text-gray-400 text-md text-center"
  >
    คุณยังไม่ได้เพิ่มรายชื่อ
  </p>
  ...
  <div class="flow-root">
    <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
      <li
        v-for="(member, index) in additionalFamilyList"
        :key="member.id"
        class="py-3 sm:py-4"
      >
        <div class="flex items-center">
          ...
          >
            <button
              @click="removeAdditionalFamilyMember(index)"
              class="text-gray-500"
            >
              <!-- X Icon -->
              <svg
        ...
        <div class="flex justify-between">
          <UButton
            ...
            >ย้อนกลับ</UButton
          >
            <UButton type="submit" @click="handleSubmit">ถัดไป</UButton>
        </div>
      </UForm>
    </BPartPageBody>
...
const goToScan2 = () => {
  router.push('./beneficiary2');
};
const goToScan3 = () => {
  router.push('./beneficiary3');
};
...
const additionalFamilyList = ref([
  { id: 'นายสมศักดิ์ ดีมาก', fullName: 'บิดา' },

]);

const removeAdditionalFamilyMember = (index: number) => {
  additionalFamilyList.value.splice(index, 1);
};
